...
DuckAction::DuckAction(std::map<uint8_t, DuckPlayer>& map_personajes,
                       ...
                       uint16_t& id_weapons, std::map<uint16_t, Protection>& map_defense,
        ...
        case PICKUP: {
            for (auto& free_weapon: map_free_weapons) {
                if (personaje.isWeaponEquipped())
                    break;
                if (inRangePickUp(free_weapon.second->getXPos(), free_weapon.second->getYPos(),
                                  HEIGHT_GUN, WIDTH_GUN, personaje)) {
            ...
            for (auto& defense: map_defense) {
                if (defense.second.type == HELMET_EQUIPPED) {
                    if (personaje.getHelmet() == HELMET_EQUIPPED)
                        break;
                    if (inRangePickUp(defense.second.x_pos, defense.second.y_pos, HEIGHT_HELMET,
                                      WIDTH_HELMET, personaje)) {
                    ...
                    if (personaje.getArmor() == ARMOR_EQUIPPED)
                        break;
                    if (inRangePickUp(defense.second.x_pos, defense.second.y_pos, HEIGHT_ARMOR,
                                      WIDTH_ARMOR, personaje)) {
                ...
                if (pick &&
                    respawn_defense_points.find(defense.first) != respawn_defense_points.end()) {
...
bool DuckAction::inRangePickUp(float x_pos, float y_pos, float HEIGHT, float WIDTH,
                               DuckPlayer& personaje) {

    return personaje.getXPos() + DUCK_WIDTH >= x_pos && personaje.getXPos() <= x_pos + WIDTH &&
           personaje.getYPos() + DUCK_HEIGHT >= y_pos && personaje.getYPos() <= y_pos + HEIGHT;
}
