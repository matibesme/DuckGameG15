...
GameLoop::GameLoop(std::shared_ptr<BlockingQueue<CommandClient>>& queue_comandos, bool& end_game,
        ...
        duck_action(map_personajes, map_free_weapons, respawn_weapon_points,
                    time_weapon_last_respawn, map_bullets, id_balas, id_weapons, map_defense,
                    respawn_defense_points, id_defense, time_defense_last_respawn),
        list_colors({"red", "blue", "green", "yellow", "pink", "purple", "orange", "brown", "black",
                     "white"}),
        load_game_config(factory_weapons, list_plataformas, respawn_weapon_points, map_defense,
                         respawn_defense_points, id_defense, id_weapons, id_boxes, map_free_weapons,
                         list_boxes, map_bullets, id_balas) {}
...
void GameLoop::sendCompleteScene() {
    ...
    for (auto& bullet: map_bullets) {
        DTOBullet dto_bullet = {bullet.first, bullet.second->getType(), bullet.second->getXPos(),
                                bullet.second->getYPos(), bullet.second->getDirection()};
...
void GameLoop::checkCoalition(std::unique_ptr<Bullet>& bullet) {
    for (auto& plataform: list_plataformas) {
        bullet->colisionWithPlatform(plataform.x_pos, plataform.y_pos, plataform.width,
                                     plataform.height);
    }
    for (auto& character: map_personajes) {
        bool colision = bullet->colisionWithDuck(
                character.second.getXPos(), character.second.getYPos(), DUCK_WIDTH, DUCK_HEIGHT);
        if (colision) {
    ...
    for (auto it = list_boxes.begin(); it != list_boxes.end();) {
        bool colision =
                bullet->colisionWithBox(it->getXPos(), it->getYPos(), WIDTH_BOX, HEIGHT_BOX);
        if (colision) {
...
void GameLoop::checkCoalitionDuckPlatform(DuckPlayer& personaje) {
    ...
    for (auto& platform: list_plataformas) {
        ...
        if (personaje.getXPos() + 15 >= platform.x_pos &&
            personaje.getXPos() + DUCK_WIDTH - 15 <= platform.x_pos + platform.width) {
            ...
            if (personaje.getXPos() + DUCK_WIDTH > platform.x_pos &&
                personaje.getXPos() < platform.x_pos && personaje.getDirection() == RIGHT) {
                personaje.setXPos(platform.x_pos - DUCK_WIDTH);
            } else if (personaje.getXPos() < platform.x_pos + platform.width &&
                       personaje.getXPos() > platform.x_pos && personaje.getDirection() == LEFT) {
...
void GameLoop::respawnWeapon() {
    ...
    for (auto& time: time_weapon_last_respawn) {
        if (time.second == 0) {
            ...
            map_free_weapons.emplace(time.first, factory_weapons.createWeapon(
                                                         weapon.type, weapon.x_pos, weapon.y_pos));
