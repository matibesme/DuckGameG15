...
LoadGameFile::LoadGameFile(FactoryWeapons& factory_weapons, std::list<DTOPlatform>& platforms,
                           ...
                           uint16_t& id_balas):
        factory_weapons(factory_weapons),
        platforms(platforms),
...
void LoadGameFile::loadGame() {
    ...
    for (const auto& platform: map["plataforms"]) {
        ...
        DTOPlatform dto_platform = {platform["type"].as<uint8_t>(), platform["pos_x"].as<float>(),
    ...
    for (const auto& wall: map["walls"]) {
        ...
        DTOPlatform dto_wall = {wall["type"].as<uint8_t>(), wall["pos_x"].as<float>(),
    ...
    for (const auto& weapon: map["weapon spawns"]) {
        RespawnPoint respawn_point = {weapon["pos_x"].as<float>(), weapon["pos_y"].as<float>(),
                                      weapon["type"].as<uint8_t>()};
        respawn_weapon_points.push_back(respawn_point);
        map_free_weapons.emplace(
                id_weapons++, factory_weapons.createWeapon(respawn_point.type, respawn_point.x_pos,
                                                           respawn_point.y_pos));
    }

    for (const auto& armour: map["armour spawns"]) {
        Protection protection_point = {armour["type"].as<uint8_t>(), armour["pos_x"].as<float>(),
    ...
    for (const auto& box: map["box spawns"]) {
        list_boxes.emplace_back(Boxes(TYPE_BOX, id_boxes++, box["pos_x"].as<float>(),
                                      box["pos_y"].as<float>(), 100, map_free_weapons, map_armor,
                                      map_helmet, map_bullets, id_bullets, id_weapons, id_helemets,
                                      id_armors));
    }
}