...
public:
    LoadGameFile(FactoryWeapons& factory_weapons,std::list<DTOPlatform> &platforms, std::map<uint16_t, RespawnPoint>& respawn_weapon_points,
        ...
        std::map<uint16_t, std::unique_ptr<Bullet>>& map_bullets, uint16_t& id_bullets, std::map<uint8_t, DuckPlayer>& map_personajes, std::map<uint8_t, std::string>& map_id_clientes);
    void loadGame();
};
