...
PewPewLaser::PewPewLaser(uint8_t type, uint8_t id, float x_pos, float y_pos,
  ...
  for (int i = 0; i < BEAM_COUNT; i++) {
    ...
    beams_vector.emplace_back(PEW_PEW_LASER_BULLET, i, 0, 0, damage, range,
                              spread);
  }
}
...
std::unique_ptr<Bullet> PewPewLaser::shoot(bool is_aiming_up) {
  ...
  case 3:
    ...
    if (direction == RIGHT) {
      ...
      beams_vector[0].release(x_pos - WIDTH_BULLET, y_pos + (DUCK_HEIGHT / 2),
                              direction, true);
    }
  ...
  case 2:
    ...
    if (direction == RIGHT) {
      ...
      beams_vector[1].release(x_pos - WIDTH_BULLET, y_pos + (DUCK_HEIGHT / 2),
                              direction, true);
    }
  ...
  case 1:
    ...
    if (direction == RIGHT) {
      ...
      beams_vector[2].release(x_pos - WIDTH_BULLET, y_pos + (DUCK_HEIGHT / 2),
                              direction, true);
    }
