...
std::unique_ptr<Bullet> LaserRifle::shoot(bool is_aiming_up) {
    ...
    if (is_aiming_up and direction == RIGHT) {
        actual_bullet.release(x_pos + DUCK_WIDTH - WIDTH_GUN/2, y_pos - WIDTH_BULLET , BULLET_UP, true);
    } else if (is_aiming_up and direction == LEFT){
        actual_bullet.release(x_pos + HEIGHT_GUN/2, y_pos - WIDTH_BULLET , BULLET_UP, true);
