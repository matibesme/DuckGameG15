...
Boxes::Boxes(uint8_t type, uint8_t id, float x_pos, float y_pos, int health,
             ...
             uint16_t& id_weapons, uint16_t& id_defense):
...
void Boxes::generateRandomContent() {
    ...
    switch (random_number) {
        case 1:
            break;
        case 2:
            map_free_weapons.emplace(id_weapons++, factory_weapons.generateRandomWeapon(
                                                           x_pos, y_pos + HEIGHT_BOX - HEIGHT_GUN));
            break;
        case 3:
            map_defense.emplace(id_defense++, Protection{ARMOR_EQUIPPED, x_pos,
                                                         y_pos + HEIGHT_BOX - HEIGHT_ARMOR});
            break;
        case 4:
            map_defense.emplace(id_defense++, Protection{HELMET_EQUIPPED, x_pos,
                                                         y_pos + HEIGHT_BOX - HEIGHT_HELMET});
            break;
        case 5:

            map_bullets.emplace(id_balas++,
                                std::move(factory_weapons
