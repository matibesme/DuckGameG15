...
Boxes::Boxes(uint8_t type, uint8_t id, float x_pos, float y_pos, int health,
    std::map<uint16_t,std::shared_ptr<Weapon>>& map_free_weapons, std::map<uint16_t, Protection>& map_defense,
...
void Boxes::generateRandomContent() {
    ...
    switch (random_number) {
        ...
        case 2:
            ...
            break;
        case 3:
            map_defense.emplace(id_defense++, Protection{ARMOR_EQUIPPED , x_pos, y_pos+HEIGHT_BOX-HEIGHT_ARMOR});
            break;
        case 4:
            map_defense.emplace(id_defense++, Protection{HELMET_EQUIPPED, x_pos, y_pos+HEIGHT_BOX-HEIGHT_HELMET});
            break;
        case 5:
