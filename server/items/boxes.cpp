...
Boxes::Boxes(uint8_t type, uint8_t id, float x_pos, float y_pos, int health,
             ...
             std::map<uint16_t, Protection>& map_armor, std::map<uint16_t, Protection>& map_helmet,
             std::map<uint16_t, std::unique_ptr<Bullet>>& map_bullets, uint16_t& id_balas,
             uint16_t& id_weapons, uint16_t& id_helemets, uint16_t& id_armors):
    ...
    switch (random_number) {
        ...
        case 2:
            map_free_weapons.emplace(id_weapons++,
                                     factory_weapons.generateRandomWeapon(x_pos, y_pos));
            break;
        case 3:
        ...
        case 5:
            ...
            map_bullets.emplace(
                    id_balas++,
                    std::move(factory_weapons.createWeapon(GRANADA_GUN, x_pos, y_pos)->shoot()));
        default:
            break;
    }
}
