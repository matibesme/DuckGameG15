...
GameRenderer::GameRenderer(Graficos& graficos, std::list<DTOPlatform>& platform,
    ...
    for (auto& platformStruct: platform) {
        platforms.emplace_back(platformStruct.x_pos, platformStruct.y_pos, graficos,
                               platformStruct.type, platformStruct.width, platformStruct.height);
    }
...
void GameRenderer::dibujar(Renderer& renderer, GameState& command) {
    ...
    SDL2pp::Texture textureDeTodo(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET,
    ...
    for (auto& armor: armors) armor.draw(false, renderer, false, false);
    for (auto& helmet: helmets) helmet.draw(false, renderer, false, false);
    ...
    // Por ahora no hago zoom renderer.Copy(textureDeTodo, SDL2pp::Optional<SDL2pp::Rect>(),
    // SDL2pp::Optional<SDL2pp::Rect>());
    renderer.Present();
}
...
void GameRenderer::actualizarElementos(const GameState& command) {
    ...
    for (auto it = ducks.begin(); it != ducks.end();) {
        auto duckInCommand = std::find_if(
                command.lista_patos.begin(), command.lista_patos.end(),
                [it](const DTODuck& duckStruct) { return duckStruct.id == it->getId(); });
        if (duckInCommand != command.lista_patos.end()) {
            ...
            it->update(duckInCommand->y_pos, duckInCommand->x_pos, duckInCommand->typeOfMove,
                       duckInCommand->typeOfGun, duckInCommand->armor, duckInCommand->helmet,
    ...
    for (const auto& duckStruct: command.lista_patos) {
        auto it = std::find_if(ducks.begin(), ducks.end(), [&duckStruct](const ClientDuck& duck) {
            return duck.getId() == duckStruct.id;
        });
        if (it == ducks.end()) {
            ducks.emplace_back(duckStruct.id, duckStruct.x_pos, duckStruct.y_pos,
                               duckStruct.typeOfGun, duckStruct.typeOfMove, duckStruct.color,
                               graficos);
        }
    }
    ...
    for (const auto& bulletStruct: command.lista_balas) {
        auto it =
                std::find_if(bullets.begin(), bullets.end(), [&bulletStruct](const Bullet& bullet) {
    ...
    for (auto it = bullets.begin(); it != bullets.end();) {
        auto bulletInCommand = std::find_if(
                command.lista_balas.begin(), command.lista_balas.end(),
                [it](const DTOBullet& bulletStruct) { return bulletStruct.id == it->getId(); });
    ...
    for (const auto& weaponStruct: command.lista_guns) {
        auto it = std::find_if(guns.begin(), guns.end(), [&weaponStruct](Gun& weapon) {
    ...
    for (const auto& armorStruct: command.lista_armors) {
        auto it = std::find_if(armors.begin(), armors.end(), [&armorStruct](Armor& armor) {
    ...
    for (const auto& helmetStruct: command.lista_helmets) {
        auto it = std::find_if(helmets.begin(), helmets.end(), [&helmetStruct](Helmet& helmet) {
    ...
    for (auto it = boxes.begin(); it != boxes.end();) {
        auto boxInCommand = std::find_if(
                ...
                [it](const DTOBoxes& boxStruct) { return boxStruct.id == it->getId(); });
    ...
    for (const auto& boxStruct: command.lista_boxes) {
        auto it = std::find_if(boxes.begin(), boxes.end(),
                               [&boxStruct](Box& box) { return box.getId() == boxStruct.id; });
        if (it == boxes.end()) {
            boxes.emplace_back(boxStruct.id, boxStruct.x_pos, boxStruct.y_pos, graficos);
        }
    }
...
void GameRenderer::drawBackground(const uint8_t background_id) {
    ...
    SDL2pp::Texture background(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET,
                               SCENE_WIDTH, SCENE_HEIGHT);
