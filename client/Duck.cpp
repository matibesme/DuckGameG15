...
Duck::Duck(float x_pos, float y_pos, uint8_t typeOfMove, uint8_t gunEquipped, Graficos& graficos)
        : positionX(x_pos), positionY(y_pos), graficos(graficos),
          numSprite(0), initialY(y_pos), gun(graficos),
          isFlipped(false), typeOfGun(gunEquipped)
          //texture(graficos.LoadTexture(DATA_PATH "/whiteDuck.png"))
          {

    gun.setGun(gunEquipped);  // Configura el arma al inicializar el pato

    // Lógica de actualización de tipo de movimiento y orientación
    if(typeOfMove != STILL) {
