...
ColorScene::ColorScene(const std::map<std::string, std::string>& playersColors, SDL_Renderer& renderer)
...
void ColorScene::RenderTitle() {
  ...
  SDL_Surface* textSurface = TTF_RenderText_Blended(font, "Duck Colors", textColor);
  SDL_Texture* textTexture = SDL_CreateTextureFromSurface(&renderer, textSurface);

  // Fondo negro con un margen mayor
  SDL_Rect backgroundRect = { (windowWidth - textSurface->w) / 2 - 20, 20 - 10, textSurface->w + 40, textSurface->h + 20 };
...
void ColorScene::RenderColorTable() {
  ...
  for (const auto& [name, color] : playersColors) {
    ...
    SDL_Rect backgroundRect = { (windowWidth - nameSurface->w) / 2 - 20, yOffset - 10, nameSurface->w + 40, nameSurface->h + 20 };
    SDL_SetRenderDrawColor(&renderer, backgroundColor.r, backgroundColor.g, backgroundColor.b, backgroundColor.a);
    SDL_RenderFillRect(&renderer, &backgroundRect);
