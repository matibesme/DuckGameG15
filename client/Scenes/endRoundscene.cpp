...
EndRoundScene::EndRoundScene(std::map<std::string, uint8_t> &players,
    ...
    : players(players),
      renderer(renderer) { // Se pasa el renderer al constructor
  // Inicializar SDL
  if (TTF_Init() == -1) {
...
void EndRoundScene::RenderResultsTable() {
  ...
  int maxYPosition =
      yOffset + (maxResults - 1) *
                    lineSpacing; // Ajustar la posición Y del último resultado
  ...
  for (const auto &[name, wins] : players) {
    ...
    yOffset +=
        lineSpacing; // Ajustar el desplazamiento para el siguiente resultado

    // Si hemos alcanzado el máximo número de resultados, detener la
    // renderización
