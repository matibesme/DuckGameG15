...
Sound::Sound() {
  if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) < 0) {
    std::cerr << "Error al inicializar SDL_mixer: " << Mix_GetError()
              << std::endl;
  }
}
...
void Sound::reproducirEfecto(const std::string &archivo) {
  ajustarVolumenEfectos(); // Ajusta el volumen basado en la cantidad de sonidos
                           // activos
  efectoSonido = Mix_LoadWAV(archivo.c_str());
  if (!efectoSonido) {
    std::cerr << "Error al cargar efecto de sonido: " << Mix_GetError()
              << std::endl;
  } else {
    Mix_VolumeChunk(efectoSonido,
                    MIX_MAX_VOLUME); // Establece volumen al máximo inicialmente
