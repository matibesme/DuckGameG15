...
ClientDuck::ClientDuck(uint8_t id, float x_pos, float y_pos, uint8_t gunEquipped,
                       uint8_t typeOfMove, std::string color, Graficos& graficos):
        idDuck(id),
        positionX(x_pos),
        positionY(y_pos),
        graficos(graficos),
        numSprite(0),
        gun(graficos, positionX + (2 * DUCK_WIDTH / 5), positionY + DUCK_HEIGHT / 2, gunEquipped),
        isFlipped(false),
        ...
        helmet(graficos, positionX, positionY),
        armorEquipped(false),
...
void ClientDuck::applyColor(SDL2pp::Renderer& renderer, const std::string& color) {
    ...
    Uint32 newColor =
            SDL_MapRGBA(loadedSurface->format, colorDuck.r, colorDuck.g, colorDuck.b, colorDuck.a);
