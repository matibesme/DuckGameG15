...
void Armor::draw(bool isFliped, SDL2pp::Renderer& renderer) {
    ...
    //if (is_equipped) {
        if (isFliped)
            renderer.Copy(texture, SDL2pp::NullOpt, Rect(pos_x - DUCK_WIDTH_CLIENT / 9, pos_y, WIDTH_ARMOR, HEIGHT_ARMOR), 0.0, SDL2pp::NullOpt, SDL_FLIP_HORIZONTAL);
        else
            renderer.Copy(texture, SDL2pp::NullOpt, Rect(pos_x - DUCK_WIDTH_CLIENT / 9 , pos_y, WIDTH_ARMOR, HEIGHT_ARMOR));
    //}else{
