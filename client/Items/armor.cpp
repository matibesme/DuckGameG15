...
void Armor::draw(bool isFliped, SDL2pp::Renderer& renderer, bool isOnGround, bool isOnDuck) {
        ...
        if(isOnGround) {
            if (isFliped)
                renderer.Copy(texture, SDL2pp::NullOpt,
                              Rect(pos_x + DUCK_WIDTH / 6, pos_y + DUCK_HEIGHT / 4, WIDTH_ARMOR, HEIGHT_ARMOR),
                              90.0, SDL2pp::NullOpt, SDL_FLIP_HORIZONTAL);
            else
                renderer.Copy(texture, SDL2pp::NullOpt,
                              Rect(pos_x + DUCK_WIDTH / 4, pos_y + DUCK_HEIGHT / 4, WIDTH_ARMOR, HEIGHT_ARMOR),
                              270.0);
        }
        else {
            if (isFliped)
                renderer.Copy(texture, SDL2pp::NullOpt, Rect(pos_x + DUCK_WIDTH / 9 + 1, pos_y + 3, WIDTH_ARMOR, HEIGHT_ARMOR),
