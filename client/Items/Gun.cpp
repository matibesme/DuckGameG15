...
void Gun::draw(bool isFliped, SDL2pp::Renderer &renderer) {
  ...
  actualizarTextura(texture_path_equipped, texture_path_not_equipped, with,
                    height);
  ...
  if (is_equiped) {
    ...
    if (isFliped) // no tendría que usarlo por que estaría en el piso pero por
      ...
      if (typeOfGun == BANANA_GUN){
        ...
        renderer.Copy(texture_not_equipped, srcRect, destRect);
      }
...
void Gun::actualizarTextura(const char *&texture_equipped,
