...
void EsperaScene::RenderMessage() {
    ...
    SDL_Color backgroundColor = {0, 0, 0, 200}; // Negro semi-transparente para el fondo

    // Renderizar primera línea del texto
    SDL_Surface *line1Surface = TTF_RenderText_Blended(font, messageLine1, textColor);
    if (!line1Surface) {
    ...
    if (!line1Texture) {
        std::cerr << "Error al crear textura de la línea 1: " << SDL_GetError() << std::endl;
        SDL_FreeSurface(line1Surface);
        return;
    }

    // Renderizar segunda línea del texto
    SDL_Surface *line2Surface = TTF_RenderText_Blended(font, messageLine2, textColor);
    if (!line2Surface) {
        std::cerr << "Error al renderizar texto: " << TTF_GetError() << std::endl;
    ...
    if (!line2Texture) {
        std::cerr << "Error al crear textura de la línea 2: " << SDL_GetError() << std::endl;
        SDL_DestroyTexture(line1Texture);
        SDL_FreeSurface(line1Surface);
    ...
    SDL_Rect line1Rect = {
            ...
            (SCENE_HEIGHT - line1Surface->h) / 2 - 20, // Centrar verticalmente (ajuste para la primera línea)
            line1Surface->w,
            line1Surface->h
    };
    ...
    SDL_Rect line2Rect = {
            ...
            (SCENE_HEIGHT - line2Surface->h) / 2 + 20, // Centrar verticalmente (ajuste para la segunda línea)
            line2Surface->w,
            line2Surface->h
    };

    SDL_Rect backgroundRect = {
            std::min(line1Rect.x, line2Rect.x) - 20, // Extender el fondo hasta el texto más a la izquierda
            line1Rect.y - 10, // Extender el fondo por encima de la primera línea
            std::max(line1Rect.w, line2Rect.w) + 40, // Extender el ancho al texto más ancho
