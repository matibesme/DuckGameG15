...
Protocolo::Protocolo(Socket &socket_servidor)
...
void Protocolo::sendString(const std::string &player_name,
                           bool &is_socket_close) {
  uint16_t tam_player_name = htons(player_name.size());
  socket_servidor.sendall(reinterpret_cast<void *>(&tam_player_name), 2,
                          &is_socket_close);
  socket_servidor.sendall(player_name.data(), player_name.size(),
                          &is_socket_close);
  checkSocketClose(is_socket_close);
}
...
std::string Protocolo::receiveString(bool &is_socket_close) {
  ...
  socket_servidor.recvall(reinterpret_cast<void *>(&tam_player_name), 2,
                          &is_socket_close);
  ...
  socket_servidor.recvall(player_name.data(), tam_player_name_bg,
                          &is_socket_close);
...
void Protocolo::sendFloat(float float_to_send, bool &is_socket_close) {
  ...
  socket_servidor.sendall(&network_order, sizeof(network_order),
                          &is_socket_close);
...
float Protocolo::receiveFloat(bool &is_socket_close) {
  ...
  socket_servidor.recvall(&network_order, sizeof(network_order),
                          &is_socket_close);
