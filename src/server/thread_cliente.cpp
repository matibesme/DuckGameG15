...
ThreadCliente::ThreadCliente(Socket peer, uint8_t id, LobbyPartidas &lobby)
    :
      ...
      protocolo(std::move(peer), dead_connection, id), id(id),
...
void ThreadCliente::sendAction(const GameState &action) {
...
std::shared_ptr<BlockingQueue<GameState>> &ThreadCliente::getQueueSender() {
  return queue_sender;
}
