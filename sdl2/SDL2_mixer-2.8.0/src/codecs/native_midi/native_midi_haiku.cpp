...
protected:
  ...
  void SprayEvent(MIDIEvent *ev, uint32 time) {
    ...
    case B_CONTROL_CHANGE:
      SprayControlChange((ev->status & 0x0F) + 1, ev->data[0], ev->data[1],
                         time);
      break;
    ...
    case 0xF:
      ...
      case B_SYSTEM_RESET:
        if (ev->data[0] == 0x51 && ev->data[1] == 0x03) {
          ...
          int val = (ev->extraData[0] << 16) | (ev->extraData[1] << 8) |
                    ev->extraData[2];
...
NativeMidiSong *native_midi_loadsong_RW(SDL_RWops *src, int freesrc) {
  ...
  if (res != B_OK) {
    snprintf(lasterr, sizeof lasterr, "Cannot Import() midi file: status_t=%d",
             res);
    delete song->store;
    delete song;
    return NULL;
...
const char *native_midi_error(void) { return lasterr; }
