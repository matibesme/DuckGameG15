...
#if defined(MODPLUG_BUILD) &&                                                  \
    defined(DLL_EXPORT) /* building libmodplug as a dll for windows */
#define MODPLUG_EXPORT __declspec(dllexport)
#elif defined(MODPLUG_BUILD) ||                                                \
    defined(                                                                   \
        MODPLUG_STATIC) /* building or using static libmodplug for windows */
#define MODPLUG_EXPORT
#else
#define MODPLUG_EXPORT                                                         \
  __declspec(dllimport) /* using libmodplug dll for windows */
#endif
#elif defined(__OS2__) && defined(__WATCOMC__)
#if defined(MODPLUG_BUILD) &&                                                  \
    defined(__SW_BD) /* building libmodplug as a dll for os/2 */
#define MODPLUG_EXPORT __declspec(dllexport)
#else
 ...
 * complete file, and [size] should be the size of that block. Return the loaded
 * mod file on success, or NULL on failure. */
 ...
 * ModPlugFile structure and will remain valid until you unload the file. */
 ...
 * mix very well.  Some mods will be missing instruments for a short time after
 * a seek, as ModPlug does not scan the sequence backwards to find out which
 * instruments were supposed to be playing at that time.  (Doing so would be
 * difficult and not very reliable.)  Also, note that seeking is not very exact
 * in some mods -- especially those for which ModPlug_GetLength() does not
...
typedef struct _ModPlug_Settings {
  ...
  int mMaxMixChannels;   /* Maximum number of mixing channels (polyphony), 32 -
                            256 */
 ...
 * bits-per-sample, sampling rate, and loop count, will take effect immediately.
 * Those options which don't take effect immediately will take effect the next
 * time you load a mod. */
...
MODPLUG_EXPORT void ModPlug_SetMasterVolume(ModPlugFile *file,
                                            unsigned int cvol);
...
MODPLUG_EXPORT unsigned int ModPlug_SampleName(ModPlugFile *file,
                                               unsigned int qual, char *buff);
MODPLUG_EXPORT unsigned int
ModPlug_InstrumentName(ModPlugFile *file, unsigned int qual, char *buff);

/*
 * Retrieve pattern note-data
 */
MODPLUG_EXPORT ModPlugNote *ModPlug_GetPattern(ModPlugFile *file, int pattern,
 ...
 */
MODPLUG_EXPORT void ModPlug_InitMixerCallback(ModPlugFile *file,
