...
#define WAVPACK_MAX_CHANS                                                      \
  4096 // max channels handled by WavPack format & library

// This sets the maximum number of channels that the current WavPack CLI
// applications accept. It's somewhat arbitrary because the actual WavPack
// format and library can handle up to 4096 channels. However, anything beyond
// 256 channels is obviously a niche case and is not well tested, so this lower
// limit is defined for now.
...
#define QMODE_EVEN_BYTE_DEPTH                                                  \
  0x2000 // user specified to force even byte bit-depth
