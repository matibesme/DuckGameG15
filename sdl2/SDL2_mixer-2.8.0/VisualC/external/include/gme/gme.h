...
#define GME_VERSION                                                            \
  0x000603 /* 1 byte major, 1 byte minor, 1 byte patch-level */
 ...
 * @since 0.6.2
 */
BLARGG_EXPORT void gme_set_autoload_playback_limit(Music_Emu *,
                                                   int do_autoload_limit);
...
BLARGG_EXPORT gme_err_t gme_track_info(Music_Emu const *, gme_info_t **out,
                                       int track);
...
extern BLARGG_EXPORT const gme_type_t gme_ay_type, gme_gbs_type, gme_gym_type,
    gme_hes_type, gme_kss_type, gme_nsf_type, gme_nsfe_type, gme_sap_type,
    gme_spc_type, gme_vgm_type, gme_vgz_type;
...
linked to this library to support new music types without having to be updated.
*/
...
BLARGG_EXPORT gme_err_t gme_open_data(void const *data, long size,
...
BLARGG_EXPORT gme_err_t gme_load_custom(Music_Emu *, gme_reader_t,
                                        long file_size, void *your_data);

/* Load m3u playlist file from memory (must be done after loading music) */
BLARGG_EXPORT gme_err_t gme_load_m3u_data(Music_Emu *, void const *data,
                                          long size);
