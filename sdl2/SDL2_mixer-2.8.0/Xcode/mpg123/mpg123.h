...
/* Simplified large file handling.
   ...
   conflicting large file setup (application that uses 32 bit offsets with
   library that uses 64 bits). While that was perfectly fine in an environment
   where there is one incarnation of the library, it hurt GNU/Linux and Solaris
   systems with multilib where the distribution fails to provide the correct
   header matching the 32 bit library (where large files need explicit support)
   or the 64 bit library (where there is no distinction).

        New approach: When the app defines _FILE_OFFSET_BITS, it wants
   non-default large file support, and thus functions with added suffix
   (mpg123_open_64). Any mismatch will be caught at link time because of the
   _FILE_OFFSET_BITS setting used when building libmpg123. Plus, there's dual
   mode large file support in mpg123 since 1.12 now. Link failure is not the
   expected outcome of any half-sane usage anymore.

        More complication: What about client code defining _LARGEFILE64_SOURCE?
   It might want direct access to the _64 functions, along with the ones without
   suffix. Well, that's possible now via defining MPG123_NO_LARGENAME and
   MPG123_LARGESUFFIX, respectively, for disabling or enforcing the suffix
   names.
*/

/*
        Now, the renaming of large file aware functions.
        By default, it appends underscore _FILE_OFFSET_BITS (so, mpg123_seek_64
   for mpg123_seek), if _FILE_OFFSET_BITS is defined. You can force a different
   suffix via MPG123_LARGESUFFIX (that must include the underscore), or you can
   just disable the whole mess by defining MPG123_NO_LARGENAME.
*/
#if (!defined MPG123_NO_LARGENAME) &&                                          \
    ((defined _FILE_OFFSET_BITS) || (defined MPG123_LARGESUFFIX))
...
#define mpg123_replace_reader_handle                                           \
 ...
 * The parameters of handles have workable defaults, you only have to tune them
 * when you want to tune something;-) Tip: Use a RVA setting...
 *
 * @{
 */
...
/** Opaque structure for the libmpg123 decoder handle.
 *  Most functions take a pointer to a mpg123_handle as first argument and
 * operate on its data in an object-oriented manner.
 */
...
enum mpg123_parms {
  MPG123_VERBOSE = 0, /**< set verbosity value for enabling messages to stderr,
                         >= 0 makes sense (integer) */
  ...
  MPG123_ICY_INTERVAL,  /**< Stream contains ICY metadata with this interval
                           (integer).  Make sure to set this _before_ opening a
                           stream.*/
  MPG123_OUTSCALE,     /**< the scale for output samples (amplitude - integer or
                          float according to mpg123 output format, normally integer)
                        */
  MPG123_TIMEOUT,      /**< timeout for reading from a stream (not supported on
                          win32, integer) */
  MPG123_REMOVE_FLAGS, /**< remove some flags (inverse of MPG123_ADD_FLAGS,
                          integer) */
  MPG123_RESYNC_LIMIT, /**< Try resync on frame parsing for that many bytes or
                          until end of stream (<0 ... integer). This can enlarge
                          the limit for skipping junk on beginning, too (but not
                          reduce it).  */
  MPG123_INDEX_SIZE /**< Set the frame index size (if supported). Values <0 mean
                       that the index is allowed to grow dynamically in these
                       steps (in positive direction, of course) -- Use this when
                       you really want a full index with every individual frame.
                     */
  ,
  MPG123_PREFRAMES /**< Decode/ignore that many frames in advance for layer 3.
                      This is needed to fill bit reservoir after seeking, for
                      example (but also at least one frame in advance is needed
                      to have all "normal" data for layer 3). Give a positive
                      integer value, please.*/
  ,
  MPG123_FEEDPOOL /**< For feeder mode, keep that many buffers in a pool to
                     avoid frequent malloc/free. The pool is allocated on
                     mpg123_open_feed(). If you change this parameter
                     afterwards, you can trigger growth and shrinkage during
                     decoding. The default value could change any time. If you
                     care about this, then set it. (integer) */
  ,
  MPG123_FEEDBUFFER /**< Minimal size of one internal feeder buffer, again, the
                       default value is subject to change. (integer) */
  ,
  MPG123_FREEFORMAT_SIZE /**< Tell the parser a free-format frame size to
                          * avoid read-ahead to get it. A value of -1 (default)
                          * means that the parser will determine it. The
                          * parameter value is applied during decoder setup for
                          * a freshly opened stream only.
                          */
};
...
enum mpg123_param_flags {
  MPG123_FORCE_MONO =
      0x7 /**<     0111 Force some mono mode: This is a test bitmask for seeing
             if any mono forcing is active. */
  ,
  ...
  ,
  MPG123_QUIET =
      0x20 /**< 00100000 Suppress any printouts (overrules verbose). */
  ,
  MPG123_GAPLESS = 0x40 /**< 01000000 Enable gapless decoding (default on if
                           libmpg123 has support). */
  ,
  MPG123_NO_RESYNC = 0x80 /**< 10000000 Disable resync stream after error. */
  ,
  MPG123_SEEKBUFFER =
      0x100 /**< 000100000000 Enable small buffer on non-seekable streams to
               allow some peek-ahead (for better MPEG sync). */
  ,
  MPG123_FUZZY =
      0x200 /**< 001000000000 Enable fuzzy seeks (guessing byte offsets or using
               approximate seek points from Xing TOC) */
  ,
  MPG123_FORCE_FLOAT =
      0x400 /**< 010000000000 Force floating point output (32 or 64 bits depends
               on mpg123 internal precision). */
  ,
  MPG123_PLAIN_ID3TEXT =
      0x800 /**< 100000000000 Do not translate ID3 text data to UTF-8. ID3
               strings will contain the raw text data, with the first byte
               containing the ID3 encoding code. */
  ,
  MPG123_IGNORE_STREAMLENGTH =
      0x1000 /**< 1000000000000 Ignore any stream length information contained
                in the stream, which can be contained in a 'TLEN' frame of an
                ID3v2 tag or a Xing tag */
  ,
  MPG123_SKIP_ID3V2 =
      0x2000 /**< 10 0000 0000 0000 Do not parse ID3v2 tags, just skip them. */
  ,
  MPG123_IGNORE_INFOFRAME =
      0x4000 /**< 100 0000 0000 0000 Do not parse the LAME/Xing info frame,
                treat it as normal MPEG data. */
  ,
  MPG123_AUTO_RESAMPLE =
      0x8000 /**< 1000 0000 0000 0000 Allow automatic internal resampling of any
                kind (default on if supported). Especially when going lowlevel
                with replacing output buffer, you might want to unset this flag.
                Setting MPG123_DOWNSAMPLE or MPG123_FORCE_RATE will override
                this. */
  ,
  MPG123_PICTURE = 0x10000 /**< 17th bit: Enable storage of pictures from tags
                              (ID3v2 APIC). */
  ,
  MPG123_NO_PEEK_END =
      0x20000 /**< 18th bit: Do not seek to the end of
               *  the stream in order to probe
               *  the stream length and search for the id3v1 field. This also
               * means the file size is unknown unless set using
               * mpg123_set_filesize() and the stream is assumed as non-seekable
               * unless overridden.
               */
  ,
  MPG123_FORCE_SEEKABLE =
      0x40000 /**< 19th bit: Force the stream to be seekable. */
  ,
  MPG123_STORE_RAW_ID3 = 0x80000 /**< store raw ID3 data (even if skipping) */
  ,
  MPG123_FORCE_ENDIAN =
      0x100000 /**< Enforce endianess of output samples.
                *  This is not reflected in the format codes. If this flag is
                * set along with MPG123_BIG_ENDIAN, MPG123_ENC_SIGNED16 means
                * s16be, without MPG123_BIG_ENDIAN, it means s16le. Normal
                * operation without MPG123_FORCE_ENDIAN produces output in
                * native byte order.
                */
  ,
  ...
  ,
  MPG123_NO_READAHEAD =
      0x400000 /**< Disable read-ahead in parser. If
                * you know you provide full frames to the feeder API, this
                * enables decoder output from the first one on, instead of
                * having to wait for the next frame to confirm that the stream
                * is healthy. It also disables free format support unless you
                * provide a frame size using MPG123_FREEFORMAT_SIZE.
                */
  ,
  MPG123_FLOAT_FALLBACK =
      0x800000 /**< Consider floating point output encoding only after
                * trying other (possibly downsampled) rates and encodings first.
                * This is to support efficient playback where floating point
                * output is only configured for an external resampler, bypassing
                * that resampler when the desired rate can be produced directly.
                * This is enabled by default to be closer to older versions of
                * libmpg123 which did not enable float automatically at all. If
                * disabled, float is considered after the 16 bit default and
                * higher-bit integer encodings for any rate. */
  ,
  MPG123_NO_FRANKENSTEIN =
      0x1000000 /**< Disable support for Frankenstein streams
                 * (different MPEG streams stiched together). Do not accept
                 * serious change of MPEG header inside a single stream. With
                 * this flag, the audio output format cannot change during
                 * decoding unless you open a new stream. This also stops
                 * decoding after an announced end of stream (Info header
                 * contained a number of frames and this number has been
                 * reached). This makes your MP3 files behave more like ordinary
                 * media files with defined structure, rather than stream dumps
                 * with some sugar. */
};
...
enum mpg123_param_rva {
  ...
  ,
  MPG123_RVA_MAX =
      MPG123_RVA_ALBUM /**< The maximum RVA code, may increase in future. */
};
 ...
 */
MPG123_EXPORT int mpg123_param(mpg123_handle *mh, enum mpg123_parms type,
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_param2(mpg123_handle *mh, int type, long value,
                                double fvalue);
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getparam(mpg123_handle *mh, enum mpg123_parms type,
                                  long *value, double *fvalue);
#endif
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getparam2(mpg123_handle *mh, int type, long *value,
                                   double *fvalue);

/** Feature set available for query with mpg123_feature. */
enum mpg123_feature_set {
  MPG123_FEATURE_ABI_UTF8OPEN = 0 /**< mpg123 expects path names to be given in
                                     UTF-8 encoding instead of plain native. */
  ,
  ...
  ,
  MPG123_FEATURE_INDEX /**< support for building a frame index for accurate
                          seeking */
  ,
...
enum mpg123_errors {
  ...
  MPG123_NEW_FORMAT =
      -11, /**< Message: Output format will be different on next call. Note that
              some libmpg123 versions between 1.4.3 and 1.8.0 insist on you
              calling mpg123_getformat() after getting this message code. Newer
              verisons behave like advertised: You have the chance to call
              mpg123_getformat(), but you can also just continue decoding and
              get your data. */
  MPG123_NEED_MORE =
      -10, /**< Message: For feed reader: "Feed me more!" (call mpg123_feed() or
              mpg123_decode() with some new input data). */
  ...
  MPG123_ERR_16TO8TABLE, /**< Unable to allocate memory for 16 to 8 converter
                            table! */
  ...
  MPG123_BAD_INDEX_PAR,    /**< Bad parameters to mpg123_index() and
                              mpg123_set_index() */
  ...
  MPG123_NULL_POINTER,      /**< You gave a null pointer somewhere where you
                               shouldn't have. */
  ...
  ,
  MPG123_LFS_OVERFLOW /**< Offset value overflow during translation of large
                         file API calls -- your client program cannot handle
                         that large file. */
  ,
 ...
 * Perhaps the most prominent feature of mpg123: You have several (optimized)
 * decoders to choose from (on x86 and PPC (MacOS) systems, that is).
 *
 * @{
 */

/** Get available decoder list.
 *  \return NULL-terminated array of generally available decoder names (plain
 * 8bit ASCII)
 */
 ...
 *
 *  \return NULL-terminated array of the decoders supported by the CPU (plain
 * 8bit ASCII)
 */
 ...
 *  Note that this can return a decoder that is only active in the hidden and
 * not available as decoder choice from the outside. \param mh handle \return
 ...
 * If you do feel confused and just want to decode your normal MPEG audio files
 * that don't alter properties in the middle, just use mpg123_open_fixed() with
 * a fixed encoding and channel count and forget about a matrix of audio
 * formats. If you want to get funky, read ahead ...
 *
 * The mpg123 library decides what output format to use when encountering the
 * first frame in a stream, or actually any frame that is still valid but
 * differs from the frames before in the prompted output format. At such a
 * deciding point, an internal table of allowed encodings, sampling rates and
 * channel setups is consulted. According to this table, an output format is
 * chosen and the decoding engine set up accordingly (including optimized
 * routines for different output formats). This might seem unusual but it just
 * follows from the non-existence of "MPEG audio files" with defined overall
 * properties. There are streams, streams are concatenations of (semi)
 * independent frames. We store streams on disk and call them "MPEG audio
 * files", but that does not change their nature as the decoder is concerned
 * (the LAME/Xing header for gapless decoding makes things interesting again).
 *
 * To get to the point: What you do with mpg123_format() and friends is to fill
 * the internal table of allowed formats before it is used. That includes
 * removing support for some formats or adding your forced sample rate (see
 * MPG123_FORCE_RATE) that will be used with the crude internal resampler. Also
 * keep in mind that the sample encoding is just a question of choice -- the
 * MPEG frames do only indicate their native sampling rate and channel count. If
 * you want to decode to integer or float samples, 8 or 16 bit ... that is your
 * decision. In a "clean" world, libmpg123 would always decode to 32 bit float
 * and let you handle any sample conversion. But there are optimized routines
 * that work faster by directly decoding to the desired encoding / accuracy. We
 * prefer efficiency over conceptual tidyness.
 *
 * People often start out thinking that mpg123_format() should change the actual
 * decoding format on the fly. That is wrong. It only has effect on the next
 * natural change of output format, when libmpg123 will consult its format table
 * again. To make life easier, you might want to call mpg123_format_none()
 * before any thing else and then just allow one desired encoding and a limited
 * set of sample rates / channel choices that you actually intend to deal with.
 * You can force libmpg123 to decode everything to 44100 KHz, stereo, 16 bit
 * integer ... it will duplicate mono channels and even do resampling if needed
 * (unless that feature is disabled in the build, same with some encodings). But
 * I have to stress that the resampling of libmpg123 is very crude and doesn't
 * even contain any kind of "proper" interpolation.
 *
 * In any case, watch out for MPG123_NEW_FORMAT as return message from decoding
 * routines and call mpg123_getformat() to get the currently active output
 * format.
 *
 * @{
 */
 ...
 * (MPG123_ENC_SIGNED_16, not MPG123_SIGNED). \param list Store a pointer to the
 * encodings array there. \param number Store the number of encodings there. */
 ...
 * will be ignored here. \return MPG123_OK on success, MPG123_ERR if there was
 * an error. */
MPG123_EXPORT int mpg123_format(mpg123_handle *mh, long rate, int channels,
                                int encodings);
...
MPG123_EXPORT int mpg123_format2(mpg123_handle *mh, long rate, int channels,
                                 int encodings);
...
MPG123_EXPORT int mpg123_format_support(mpg123_handle *mh, long rate,
                                        int encoding);
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getformat(mpg123_handle *mh, long *rate, int *channels,
                                   int *encoding);
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getformat2(mpg123_handle *mh, long *rate,
 ...
 * Decoding/seek functions may also return message codes MPG123_DONE,
 * MPG123_NEW_FORMAT and MPG123_NEED_MORE (please read up on these on how to
 * react!).
 * @{
 */
 ...
 *  then close using mpg123_close(), or implicitly on mpg123_delete() or the
 ...
 *  If you want to decode internet streams, use mpg123_open_fd() or
 * mpg123_open_feed().
 *
 ...
 *  underneath and the path argument to libmpg123 is assumed to be encoded in
 * UTF-8. So, if you have to ask yourself which encoding is needed, the answer
 * is UTF-8, which also fits any sane modern install of Unix-like systems.
 *
 *  \param mh handle
 ...
 *  mpg123_close() will call the cleanup callback for your handle (if you gave
 * one). \param mh handle \param iohandle your handle \return MPG123_OK on
 * success
 */
MPG123_EXPORT int mpg123_open_handle(mpg123_handle *mh, void *iohandle);

/** Open a new bitstream and prepare for direct feeding
 *  This works together with mpg123_decode(); you are responsible for reading
 * and feeding the input bitstream. Also, you are expected to handle ICY
 * metadata extraction yourself. This input method does not handle
 * MPG123_ICY_INTERVAL. It does parse ID3 frames, though. \param mh handle
 *  \return MPG123_OK on success
 */
 ...
 */
MPG123_EXPORT int mpg123_read(mpg123_handle *mh, void *outmemory,
                              size_t outmemsize, size_t *done);

/** Feed data for a stream that has been opened with mpg123_open_feed().
 *  It's give and take: You provide the bytestream, mpg123 gives you the decoded
 * samples. \param mh handle \param in input buffer \param size number of input
 * bytes \return MPG123_OK or error/message code.
 */
MPG123_EXPORT int mpg123_feed(mpg123_handle *mh, const unsigned char *in,
                              size_t size);
 ...
 * query it) without taking decoded data. Think of this function being the union
 * of mpg123_read() and mpg123_feed() (which it actually is, sort of;-). You can
 * actually always decide if you want those specialized functions in separate
 * steps or one call this one here.
 *
 *  Note: The type of outmemory changed to a void pointer in mpg123 1.26.0
 *  (API version 45).
 *
...
MPG123_EXPORT int mpg123_decode(mpg123_handle *mh,
                                ...
                                void *outmemory, size_t outmemsize,
                                size_t *done);
 ...
 * audio from. \param bytes number of output bytes ready in the buffer \return
 * MPG123_OK or error/message code
 */
MPG123_EXPORT int mpg123_decode_frame(mpg123_handle *mh, off_t *num,
 ...
 * MPG123_OK or error/message code
 */
MPG123_EXPORT int mpg123_framebyframe_decode(mpg123_handle *mh, off_t *num,
                                             unsigned char **audio,
                                             size_t *bytes);
 ...
 * Together with the raw header, you can reconstruct the whole raw MPEG stream
 * without junk and meta data, or play games by actually modifying the frame
 * body data before decoding this frame (mpg123_framebyframe_decode()). A more
 * sane use would be to use this for CRC checking (see mpg123_info() and
 * MPG123_CRC), the first two bytes of the body make up the CRC16 checksum, if
 * present. You can provide NULL for a parameter pointer when you are not
 * interested in the value.
 *
 * \param mh handle
 * \param header the 4-byte MPEG header
 * \param bodydata pointer to the frame body stored in the handle (without the
 * header) \param bodybytes size of frame body in bytes (without the header)
 ...
 *    this function).
 */
MPG123_EXPORT int mpg123_framedata(mpg123_handle *mh, unsigned long *header,
                                   unsigned char **bodydata, size_t *bodybytes);
 ...
 * #MPG123_UPSPEED, #MPG123_DOWNSPEED or #MPG123_START_FRAME. The first two
 * mainly should cause trouble with NtoM resampling, but in any case with these
 * options in effect, you have to keep in mind that the sample offset is not the
 * same as counting the samples you get from decoding since mpg123 counts the
 * skipped samples, too (or the samples played twice only once)!
 *
 ...
 * \return The resulting offset >= 0 or error/message code
 */
MPG123_EXPORT off_t mpg123_feedseek(mpg123_handle *mh, off_t sampleoff,
                                    int whence, off_t *input_offset);
 ...
 * \param whence one of SEEK_SET, SEEK_CUR or SEEK_END
 * \return The resulting offset >= 0 or error/message code */
MPG123_EXPORT off_t mpg123_seek_frame(mpg123_handle *mh, off_t frameoff,
                                      int whence);
 ...
 * which is a good assumption for any sane file/stream only. \return frame
 * offset >= 0 or error/message code */
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_index(mpg123_handle *mh, off_t **offsets, off_t *step,
                               size_t *fill);

/** Set the frame index table
 *  Setting offsets to NULL and fill > 0 will allocate fill entries. Setting
 * offsets to NULL and fill to 0 will clear the index and free the allocated
 * memory used by the index. \param mh handle \param offsets pointer to the
 * index array \param step    one index byte offset advances this many MPEG
 * frames \param fill    number of recorded index offsets; size of the array
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_set_index(mpg123_handle *mh, off_t *offsets,
                                   off_t step, size_t fill);
 ...
 */
MPG123_EXPORT int mpg123_position(mpg123_handle *mh, off_t frame_offset,
...
enum mpg123_channels {
  ...
  ,
  MPG123_LR =
      0x3 /**< Both left and right channel; same as MPG123_LEFT|MPG123_RIGHT */
};
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_eq(mpg123_handle *mh, enum mpg123_channels channel,
                            int band, double val);
#endif
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_eq2(mpg123_handle *mh, int channel, int band,
                             double val);
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getvolume(mpg123_handle *mh, double *base,
                                   double *really, double *rva_db);
...
struct mpg123_frameinfo {
  ...
  enum mpg123_mode
      mode; /**< The audio mode (Mono, Stereo, Joint-stero, Dual Channel). */
  int mode_ext;  /**< The mode extension bit flag. */
  int framesize; /**< The size of the frame (in bytes, including header). */
  enum mpg123_flags flags; /**< MPEG Audio flag bits. Just now I realize that it
                              should be declared as int, not enum. It's a
                              bitwise combination of the enum values. */
...
/** Data structure for storing information about a frame of MPEG Audio without
 * enums */
struct mpg123_frameinfo2 {
  int version; /**< The MPEG version (1.0/2.0/2.5), enum mpg123_version. */
  int layer;   /**< The MPEG Audio Layer (MP1/MP2/MP3). */
  long rate;   /**< The sampling rate in Hz. */
  int mode;    /**< The audio mode (enum mpg123_mode, Mono, Stereo, Joint-stero,
                  Dual Channel). */
  int mode_ext;  /**< The mode extension bit flag. */
  int framesize; /**< The size of the frame (in bytes, including header). */
  int flags; /**< MPEG Audio flag bits. Bitwise combination of enum mpg123_flags
                values. */
  int emphasis; /**< The emphasis type. */
  int bitrate;  /**< Bitrate of the frame (kbps). */
  int abr_rate; /**< The target average bitrate. */
 ...
 *  \return MPG123_OK if the collection was enabled/disabled as desired,
 * MPG123_ERR otherwise (e.g. if the feature is disabled)
 */
 ...
 *  Useful for getting sensible track length values in feed mode or for HTTP
 * streams. \param mh handle \param size file size in bytes \return MPG123_OK on
 * success
 */
...
enum mpg123_state {
  ...
  ,
  MPG123_BUFFERFILL /**< Get fill of internal (feed) input buffer as integer
                       byte count returned as long and as double. An error is
                       returned on integer overflow while converting to (signed)
                       long, but the returned floating point value shold still
                       be fine. */
  ,
  MPG123_FRANKENSTEIN /**< Stream consists of carelessly stitched together
                         files. Seeking may yield unexpected results (also with
                         MPG123_ACCURATE, it may be confused). */
  ,
  MPG123_FRESH_DECODER /**< Decoder structure has been updated, possibly
                          indicating changed stream (integer value, 0 if false,
                          1 if true). Flag is cleared after retrieval. */
  ,
  MPG123_ENC_DELAY /** Encoder delay read from Info tag (layer III, -1 if
                      unknown). */
  ,
  MPG123_ENC_PADDING /** Encoder padding read from Info tag (layer III, -1 if
                        unknown). */
  ,
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getstate(mpg123_handle *mh, enum mpg123_state key,
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getstate2(mpg123_handle *mh, int key, long *val,
                                   double *fval);
 ...
 *  \return 0 on error, 1 on success
 */
MPG123_EXPORT int mpg123_add_substring(mpg123_string *sb, const char *stuff,
 ...
 *  \return 0 on error, 1 on success
 */
MPG123_EXPORT int mpg123_set_substring(mpg123_string *sb, const char *stuff,
                                       size_t from, size_t count);
...
enum mpg123_text_encoding {
  ...
  ,
  mpg123_text_icy = 3 /**< ICY metadata encoding, usually CP-1252 but we take it
                         as UTF-8 if it qualifies as such. */
  ,
  mpg123_text_cp1252 = 4 /**< Really CP-1252 without any guessing. */
  ,
  mpg123_text_utf16 =
      5 /**< Some UTF-16 encoding. The last of a set of leading BOMs (byte order
         * mark) rules. When there is no BOM, big endian ordering is used. Note
         * that UCS-2 qualifies as UTF-8 when you don't mess with the reserved
         * code points. If you want to decode little endian data without BOM you
         * need to prepend 0xff 0xfe yourself. */
  ,
  mpg123_text_utf16bom =
      6 /**< Just an alias for UTF-16, ID3v2 has this as distinct code. */
  ,
  mpg123_text_utf16be =
      7 /**< Another alias for UTF16 from ID3v2. Note, that, because of the mess
         * that is reality, BOMs are used if encountered. There really is not
         * much distinction between the UTF16 types for mpg123 One exception:
         * Since this is seen in ID3v2 tags, leading null bytes are skipped for
         * all other UTF16 types (we expect a BOM before real data there), not
         * so for utf16be!*/
  ,
...
enum mpg123_id3_enc {
  mpg123_id3_latin1 =
      0 /**< Note: This sometimes can mean anything in practice... */
  ,
  mpg123_id3_utf16bom =
      1 /**< UTF16, UCS-2 ... it's all the same for practical purposes. */
  ,
  mpg123_id3_utf16be =
      2 /**< Big-endian UTF-16, BOM see note for mpg123_text_utf16be. */
  ,
  mpg123_id3_utf8 = 3 /**< Our lovely overly ASCII-compatible 8 byte encoding
                         for the world. */
  ,
  mpg123_id3_enc_max =
      3 /**< Placeholder to check valid range of encoding byte. */
};
...
#ifdef MPG123_ENUM_API
/** Store text data in string, after converting to UTF-8 from indicated
 * encoding.
 *
 *  Note that this name is mapped to mpg123_store_utf8_2() instead unless
 *  MPG123_ENUM_API is defined.
 *
 *  A prominent error can be that you provided an unknown encoding value, or
 * this build of libmpg123 lacks support for certain encodings (ID3 or ICY stuff
 * missing). Also, you might want to take a bit of care with preparing the data;
 * for example, strip leading zeroes (I have seen that). \param sb  target
 * string \param enc mpg123 text encoding value \param source source buffer with
 * plain unsigned bytes (you might need to cast from signed char) \param
 * source_size number of bytes in the source buffer \return 0 on error, 1 on
 * success (on error, mpg123_free_string is called on sb)
 */
 ...
 * missing). Also, you might want to take a bit of care with preparing the data;
 * for example, strip leading zeroes (I have seen that). \param sb  target
 * string \param enc mpg123 text encoding value (enum mpg123_text_encoding)
 *  \param source source buffer with plain unsigned bytes (you might need to
 * cast from signed char) \param source_size number of bytes in the source
 * buffer \return 0 on error, 1 on success (on error, mpg123_free_string is
 * called on sb)
 */
MPG123_EXPORT int mpg123_store_utf8_2(mpg123_string *sb, int enc,
                                      const unsigned char *source,
                                      size_t source_size);
 ...
 * COMM, TXXX and USLT may have a description, only COMM and USLT have a
 * language. You should consult the ID3v2 specification for the use of the
 * various text fields
...
typedef struct {
  ...
  char id[4]; /**< The ID3v2 text field id, like TALB, TPE2, ... (4 characters,
 ...
 *  for the use of the APIC field ("frames" in ID3v2 documentation, I use
 * "fields" here to separate from MPEG frames). */
typedef struct {
...
typedef struct {
  ...
  mpg123_string
      *genre; /**< Genre String (pointer into text_list). The genre string(s)
                 may very well need postprocessing, esp. for ID3v2.3. */
  mpg123_string *comment; /**< Pointer to last encountered comment text with
                             empty description. */
 ...
 *  Also note the change ID3v1.1 did: comment[28] = 0; comment[29] =
 * track_number It is your task to support ID3v1 only or ID3v1.1 ...*/
typedef struct {
...
#define MPG123_ID3                                                             \
  0x3 /**< 0011 There is some ID3 info. Also matches 0010 or NEW_ID3. */
#define MPG123_NEW_ID3                                                         \
  0x1 /**< 0001 There is ID3 info that changed since last call to mpg123_id3.  \
       */
#define MPG123_ICY                                                             \
  0xc /**< 1100 There is some ICY info. Also matches 0100 or NEW_ICY.*/
#define MPG123_NEW_ICY                                                         \
  0x4 /**< 0100 There is ICY info that changed since last call to mpg123_icy.  \
       */

/** Query if there is (new) meta info, be it ID3 or ICY (or something new in
 * future). \param mh handle \return combination of flags, 0 on error (same as
 * "nothing new")
 */
 ...
 * corresponding data. \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_id3(mpg123_handle *mh, mpg123_id3v1 **v1,
                             mpg123_id3v2 **v2);
 ...
 *          values are set.
 */
MPG123_EXPORT int mpg123_id3_raw(mpg123_handle *mh, unsigned char **v1,
                                 size_t *v1_size, unsigned char **v2,
                                 size_t *v2_size);

/** Point icy_meta to existing data structure wich may change on any next
 * read/decode function call. \param mh handle \param icy_meta return address
 * for ICY meta string (set to NULL if nothing there) \return MPG123_OK on
 * success
 */
 ...
 *  Note that this is very similar to mpg123_store_utf8(&sb, mpg123_text_icy,
 * icy_text, strlen(icy_text+1)) . \param icy_text The input data in ICY
 * encoding \return pointer to newly allocated buffer with UTF-8 data (You
 * free() it!) */
 ...
 */
MPG123_EXPORT mpg123_handle *mpg123_parnew(mpg123_pars *mp, const char *decoder,
                                           int *error);
 ...
 * \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_fmt(mpg123_pars *mp, long rate, int channels,
                             int encodings);
 ...
 * \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_fmt2(mpg123_pars *mp, long rate, int channels,
                              int encodings);
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_par(mpg123_pars *mp, enum mpg123_parms type,
                             long value, double fvalue);
#endif
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_par2(mpg123_pars *mp, int type, long value,
                              double fvalue);
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getpar(mpg123_pars *mp, enum mpg123_parms type,
                                long *value, double *fvalue);
#endif
 ...
 *  \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_getpar2(mpg123_pars *mp, int type, long *value,
                                 double *fvalue);
 ...
 * Instead of working on it's own private buffer, mpg123 will directly use the
 * one you provide for storing decoded audio. Note that the required buffer size
 * could be bigger than expected from output encoding if libmpg123 has to
 * convert from primary decoder output (p.ex. 32 bit storage for 24 bit output).
 *
 ...
 * \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_replace_buffer(mpg123_handle *mh, void *data,
                                        size_t size);

/** The max size of one frame's decoded output with current settings.
 *  Use that to determine an appropriate minimum buffer size for decoding one
 * frame. \param mh handle \return maximum decoded data size in bytes
 */
...
MPG123_EXPORT int mpg123_replace_reader(mpg123_handle *mh,
                                        mpg123_ssize_t (*r_read)(int, void *,
                                                                 size_t),
 ...
 * \return MPG123_OK on success
 */
MPG123_EXPORT int mpg123_replace_reader_handle(
    mpg123_handle *mh, mpg123_ssize_t (*r_read)(void *, void *, size_t),
