...
main()
{
...

  {
    ...
    dlg_info(
        "But we could output and use them (e.g. as filter) in a custom "
        "handler" );
  }

  // we can also use formatting
  // the default c api uses printf semantics but by including dlg.hpp we
  // override them to a small typesafe replacement that simply replaces a given
  // sequence (see dlg.hpp for config) with the printed object.
  // Uses std::ostream with operator<< so should work for everything and is
  // extendable. You could also use your custom formatting function (like
  // libfmt) here if you wish. It is not included (anymore) to reduce bloat.
  ...
  dlg::set_handler(
      []( const struct dlg_origin& origin, const char* msg )
      {
        // don't print anything below warning messages
        // note that if this is done for performance reasons or statically
        // project-wide prefer to use the config macros since they will result
        // in zero compile and runtime overhead, this way will not
        ...
        // depending on tags/type/level or even based on the messages content
        // (probably a bad idea) we an print the output to different
        // streams/files in this case we check if the origin has an expression
        // associated, i.e. if it came from an assert call and then write it
        // without color to a file stream. everything else is dumped with color
        // to cout NOTE: the simple color switch here will lead to troubles when
        // cout is e.g. redirected to a file (but could be useful again when
        // using unix's 'less -R')
        ...
        // we could add additional switches for e.g. file/line, tags or msg
        // content

        // we call the generic output handler that will take care
        // of formatting the origin (tags/type/expression/level/file/line) and
        // color. We could print more stuff (like tags/time, see dlg/output.h)
        // but stick with some clean defaults here
  ...
  // we can also choose the level dynamically by using the macros with the 'l'
  // suffix
  dlg_assertl( dlg_level_debug,
               42 * 42 == -42 );  // should not be printed at all, level too low
  dlg_assertl( dlg_level_fatal,
               "dlg"[0] == 42 );                 // should be printed into file
  dlg_assertm( false, "Error assert message" );  // default assert level is
                                                 // error, printed to file
  ...
  dlg_assertm( true == false,
               "Assertions are printed to cout again" );     // printed to cout
  dlg_tracet( ( "filtered" ), "I am printed again, yeay" );  // printed to cout

  dlg_info(
      "If you don't like the colors here, don't despair; they may be "
      "changed!" );
  dlg_info(
      "Also make sure to check out the example_assert_log file i printed "
      "to\n" );

  dlg_fatal( "What now follows are the important bottom lines" );
  dlg_info(
      "You must be really confused right now if you don't know what this "
      "program is" );
  dlg_info( "Just read the dlg example file which uses all this to show off" );
  dlg_info(
      "Now, it is time. I'm a busy program after all. A good day m'lady or my "
      "dear sir!" );
