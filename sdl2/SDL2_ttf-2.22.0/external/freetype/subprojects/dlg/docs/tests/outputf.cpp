...
sample()
{
  ...
  dlg_infot( ( "tag1", "tag2" ),
             "We can tag our stuff. Can be used to filter/redirect messages" );
  dlg_asserttm( ( "tag3" ), 3 == 2, "The same goes for asserts" );
  dlg_info(
      "Another feature: Utf-8 printing works automatically, even for שׁǐŉďốẅś "
      "consoles" );
  dlg_fatal(
      "This one is printed bold. For more information, read the example "
      "already" );
}
...
{
  dlg::set_handler(
      [&]( const struct dlg_origin& origin, const char* str )
      {
        dlg_generic_outputf_stream( stdout, "%s%c\n", &origin, str,
                                    dlg_default_output_styles, false );
      } );
  ...
  dlg::set_handler(
      [&]( const struct dlg_origin& origin, const char* str )
      {
        dlg_generic_outputf_stream( stdout, "[%o %h:%m {%t} %f] %s%c\n",
                                    &origin, str, dlg_default_output_styles,
                                    false );
      } );

  dlg_info(
      "outputting pretty much all information now. Only message is styled" );
  sample();
  std::fprintf(
      stdout,
      "Normal, non-dlg printf message. Should not be effected by style\n" );
