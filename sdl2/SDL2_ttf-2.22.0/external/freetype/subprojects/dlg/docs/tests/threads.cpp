...
main()
{
  ...
  dlg::set_handler(
      ...
      {
        auto t = ( std::this_thread::get_id() == tid1 ) ? "thread-1: "
                                                        : "thread-2: ";
        dlg_win_init_ansi();
        dlg_fprintf( stdout, "%s %s", t,
                     dlg::generic_output( ~0u, origin, str ).c_str() );
      } );
  test();
  ...
  dlg::set_handler(
      [&]( const struct dlg_origin& origin, const char* str )
      {
        unsigned int features =
            dlg_output_file_line | dlg_output_newline | dlg_output_style;
        dlg_generic_output_stream( stdout, features, &origin, str,
